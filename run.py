...
while episodes < number_of_episodes and esc == 0:
    ...
    hider_objs = [hider1, hider2,hider3]
    seeker_objs = [seeker1]
    running = True
    x = 0
    while running:
        ...
        # Drwaing in the screen
        screen.fill((0, 0, 0))
        floor = pygame.image.load('lava.jpeg', "r") #floor
        screen.blit(floor, (800, 400))
        floor = pygame.image.load('thunder.jpg', "r") #sky
        ...
        for s in seeker_objs:
            for line in seeker1.vision:
                mx, my, px, py = line
                pygame.draw.aaline(screen, light_red, (mx, my), (px, py))

        pov_switch = pygame.draw.rect(screen, (45, 43, 23), pygame.Rect(20, 780, 180, 100))
        eye = pygame.image.load('eye.jpg', "r")
        screen.blit(eye, (20, 780))

        text_surface, rect = GAME_FONT.render(f"Number of Games Completed  = {episodes}", white)
        ...
        for s in seeker_objs:
            text_surface, rect = GAME_FONT.render(f"Seeker (Red) Rewards are {str(s.agent_seeker.total_reward)[:6]}",cyan)
